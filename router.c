...
int main (int argc, char ** argv){
	...
	int update_interval_flag_copy = 0;
	...
	while (1){
		...
		update_interval_flag_copy = update_interval_flag; 
		pthread_mutex_unlock(&lock); 
		if (update_interval_flag_copy == 1){
...
void * udp_server_thread(void * param){
	...
	while (1){
		// Wait for a packet and then use received packet to update router table
		...
		// Get neighbor cost from table, and signal that recieved an update message from this neighbor
		pthread_mutex_lock(&lock); 
		nbr_cost = get_cost_to_neighbor(recv_update_pkt.sender_id);
		signal_received_update_from_neighbor(recv_update_pkt.sender_id); 
		pthread_mutex_unlock(&lock); 
		UpdateRoutes(&recv_update_pkt, nbr_cost, arguments->router_id);
	}
...
void signal_received_update_from_neighbor(int nbr_id){
	...
	// Look for neighbor in table and reset its update timer to a new zero base
	for (i = 0; i < neighbor_table.no_nbr; i++){
		if (nbr_id == neighbor_table.neighbors[i].nbr){
			neighbor_table.neighbors[i].recv_update_flag = time(NULL); 
			return; 
		}
	}
...
void init_router(int router_id, int ne_udp_port, int router_udp_port, char * ne_hostname, int sockfd){
	...
	// Wait for init response packet and then update table
	len = sizeof(cliaddr); 
